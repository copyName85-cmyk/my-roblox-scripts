...
MainFrame.Position = UDim2.new(0.5, -150, 0.4, -220)
...
-- Отображение выбранного значения скорости
local SpeedValueLabel = Instance.new("TextLabel")
SpeedValueLabel.Name = "SpeedValueLabel"
SpeedValueLabel.Size = UDim2.new(1, 0, 0, 30)
SpeedValueLabel.Position = UDim2.new(0, 0, 0, 10)
...
-- Линия ползунка (Slider Track)
local SliderTrack = Instance.new("Frame")
SliderTrack.Name = "SliderTrack"
SliderTrack.Size = UDim2.new(0, 240, 0, 6)
SliderTrack.Position = UDim2.new(0.5, -120, 0, 55)
...
local SpeedToggleButton, SpeedToggleCircle = createToggle("SpeedToggle", "Скорость:", 85)
local JumpToggleButton, JumpToggleCircle = createToggle("JumpToggle", "Бесконечный прыжок:", 130)
local NoFallToggleButton, NoFallToggleCircle = createToggle("NoFallToggle", "Без урона от падения:", 175)
local FlyToggleButton, FlyToggleCircle = createToggle("FlyToggle", "Режим полета (Fly):", 220)
...
local EspToggleButton, EspToggleCircle = createToggle("EspToggle", "Подсветка игроков (ESP):", 355)
local MIN_SPEED = 0
local MAX_SPEED = 100
...
-- Функции базовых читов
local function applySpeed()
	local character = LocalPlayer.Character
	if character and character:FindFirstChildOfClass("Humanoid") then
		character:FindFirstChildOfClass("Humanoid").WalkSpeed = isSpeedEnabled and targetSpeed or DEFAULT_SPEED
	end
end
...
local function teleportToMouse(inputPosition)
	...
	local crosshairX, crosshairY, zoneSize = updateZonePosition()
	if math.abs(inputPosition.X - crosshairX) <= (zoneSize / 2) and math.abs(inputPosition.Y - crosshairY) <= (zoneSize / 2) then
		local camera = workspace.CurrentCamera
		local unitRay = camera:ViewportPointToRay(crosshairX, crosshairY)
		local raycastParams = RaycastParams.new()
		raycastParams.FilterType = Enum.RaycastFilterType.Exclude; raycastParams.FilterDescendantsInstances = {character}
...
local function createEspElements(player)
	...
	local function drawEsp()
		...
		highlight.Name = "EspHighlight"; highlight.FillColor = Color3.fromRGB(0, 140, 255); highlight.FillTransparency = 0.6
		highlight.OutlineColor = Color3.fromRGB(255, 255, 255); highlight.OutlineTransparency = 0.2
		highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; highlight.Enabled = isEspEnabled; highlight.Parent = rootPart

		local billboard = rootPart:FindFirstChild("EspBillboard") or Instance.new("BillboardGui")
		billboard.Name = "EspBillboard"; billboard.Size = UDim2.new(0, 200, 0, 50); billboard.AlwaysOnTop = true
		billboard.ExtentsOffset = Vector3.new(0, 3, 0); billboard.Enabled = isEspEnabled; billboard.Parent = rootPart

		local label = billboard:FindFirstChild("EspLabel") or Instance.new("TextLabel")
		label.Name = "EspLabel"; label.Size = UDim2.new(1, 0, 1, 0); label.BackgroundTransparency = 1
		label.TextColor3 = Color3.fromRGB(255, 255, 255); label.TextStrokeTransparency = 0; label.TextSize = 13
		label.Font = Enum.Font.SourceSansBold; label.Text = player.Name .. " [" .. math.round(humanoid.Health) .. " HP]"; label.Parent = billboard
	end
...
-- Цикл Heartbeat для Fly, Noclip и NoFall
...
game:GetService("RunService").Heartbeat:Connect(function()
	...
	if isNoFallEnabled and rootPart.Velocity.Y < -25 then
		humanoid:ChangeState(Enum.HumanoidStateType.Running); rootPart.Velocity = Vector3.new(rootPart.Velocity.X, 0, rootPart.Velocity.Z)
	end
...
NoclipToggleButton.MouseButton1Click:Connect(function() isNoclipEnabled = not isNoclipEnabled; toggleVisual(isNoclipEnabled, NoclipToggleButton, NoclipToggleCircle) end)
...
MinimizeButton.MouseButton1Click:Connect(function()
	...
	MainFrame.Size = isMinimized and UDim2.new(0, 300, 0, 40) or UDim2.new(0, 300, 0, 440)
	MinimizeButton.Text = isMinimized and "+" or "—"
end)

CloseButton.MouseButton1Click:Connect(function()
	isSpeedEnabled, isJumpEnabled, isNoFallEnabled, isFlyEnabled, isNoclipEnabled, isTpEnabled, isEspEnabled = false, false, false, false, false, false, false
	applySpeed(); refreshEsp(); ScreenGui:Destroy()
end)
